...
pub struct Ray {
    /// The ray origin.
    pub origin: BVHPoint3,

    /// The ray direction.
    pub direction: BVHVector3,

    /// Inverse (1/x) ray direction. Cached for use in [`AABB`] intersections.
    ///
    /// [`AABB`]: struct.AABB.html
    ///
    inv_direction: BVHVector3,
...
impl Ray {
    ...
    /// [`Ray`]: struct.Ray.html
    ///
    pub fn new(origin: BVHPoint3, direction: BVHVector3) -> Ray {
        let direction = direction.normalize();
        Ray {
            origin,
            direction,
            inv_direction: BVHVector3::new(1.0 / direction.x, 1.0 / direction.y, 1.0 / direction.z),
    ...
    /// let ray = Ray::new(origin, direction);
    ///
    /// let point1 = BVHPoint3::new(99.9,-1.0,-1.0);
    /// let point2 = BVHPoint3::new(100.1,1.0,1.0);
    ...
    pub fn intersects_triangle(
        ...
        let u = bvh_vec_dot(a_to_origin, u_vec) * inv_det;
        ...
        let v_vec = bvh_vec_cross(a_to_origin, a_to_b);

        // Calculate v parameter and test bound
        let v = bvh_vec_dot(self.direction, v_vec) * inv_det;
        // The intersection lies outside of the triangle
        if v < 0.0 || u + v > 1.0 {
            return Intersection::new(INFINITY, u, v);
        }

        let dist = bvh_vec_dot(a_to_c, v_vec) * inv_det;
...
mod tests {
    ...
    quickcheck! {
        fn test_ray_hits_triangle(a: TupleVec,
            ...
            let normal = bvh_vec_cross(u_vec, v_vec);
            ...
            let on_back_side = bvh_vec_dot(normal, ray.origin - triangle.0) <= 0.0;
